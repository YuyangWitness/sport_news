...
<head>
  ...
  <script>
    ...
    function pageInit(){
      ...
      jQuery("#list2").jqGrid(
              {
                url : '${ctx}/sys-comment/GetComment',
                datatype : "json",
                colNames : [ '评论内容','评论人','评论新闻','评论时间','操作'],
                colModel : [

                  {name : 'content',index : 'content',width : 150,align : "center"},
                  {name : 'name',index : 'name',width : 150,align : "center"},
                  {name : 'new_id',index : 'new_id',width : 150,align : "center"},
                  ...
                  {name : 'id',index : 'id',width : 150,align : "center",formatter:function(cellvalue, options, rowObject){
                    var msgdelete = '<a href="${ctx}/sys-comment/delete-comment?id='+cellvalue+'" >[删除]</a> &nbsp;&nbsp;';
                    return msgdelete;
                  }
                  }
                ],
    ...
    <div class="tab3" id="tab3">
      <dl class="call">
        <div class="medo medo1">
          <dd id="colum1"><a href="${ctx}/sys-comment/init-comment">评论管理</a></dd>
        </div>
      </dl>
    </div>
  </div>
